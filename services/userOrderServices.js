...
export const getOrderService = async (userId,search = "",page=1,limit=5) => {
...
export const cancelOrderService = async (userId,orderId,reason="")=>{
  ...
  for(const item of order.items){
    if(items.status==="cancelled")continue;
    const product = await Product.findById(item.product);
    const variant = product.variants.find((v)=>v._id.toString()===item.varientId?.toString());
    if(!variant){
      continue;
    }
...
export const cancelOrderItemService = async (userId,orderId,itemIndex,reason="")=>{
  ...
  if (product) {
    const variant = (product.variants || []).find(
      (v) => String(v.price) === String(item.price) || v.stock >= 0
    );
    if (variant) {
      variant.stock += item.quantity;
    }
